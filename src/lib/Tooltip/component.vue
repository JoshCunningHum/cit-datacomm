...
import type { TooltipValue } from "./index";
...
const data = computed<Exclude<TooltipValue, string> | undefined>(() => {
    ...
    const [first] = els;
    if (element && !element.contains(first!) && elementindex > 0)
        return undefined;
...
const offset = computed(
    () => (data.value?.offset || [16, 21]) as [number, number]
);
...
const x = computed(() => {
    const isLeft =
        (data.value?.quadrant && data.value.quadrant.includes("left")) ||
        isXOverlap.value;
...
const y = computed(() => {
    const isTop =
        (data.value?.quadrant && data.value.quadrant.includes("top")) ||
        isYOverlap.value;
...
<template>
    <div
        class="absolute bg-inherit bg top-0 left-0"
        style="z-index: 99999"
        v-if="active"
    >
