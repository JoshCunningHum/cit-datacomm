...
import { get, set, useDraggable, useWindowSize, whenever } from "@vueuse/core";
import { nextTick, onMounted, reactive, ref } from "vue";
import * as yup from "yup";
import type { QueryInfer, QueryState } from "../DynamicForm/types";
    ...
    <Teleport to="body">
        ...
        >
            <Transition name="fade">
                <div
                    v-if="!hidden"
                    class="backdrop absolute h-screen w-screen bg-surface-800/70 backdrop-blur-sm"
                    ref="backdrop"
                    @click="
                        () => {
            ...
            <Transition name="expand">
                <div
                    ...
                    v-if="!hidden"
                    ref="content"
                    class="content absolute bottom-0 left-0 right-0 top-0 h-fit w-fit min-w-[400px] rounded-md border border-surface-800 bg-surface-900 shadow-xl"
                    :class="{
                ...
                >
                    ...
                    <div
                        ref="handle"
                        class="flex items-center justify-between rounded-t-md bg-surface-950 p-6 pb-2 pt-4 text-base font-semibold"
                    >
                    ...
                    <div v-if="!!schema" class="px-6 pb-3">
                        <DynamicForm
                            ...
                            :join-labels="state.joinLabels"
                        />
                    </div>
                    <div
                        v-else
                        class="flex justify-end gap-2 px-6 pb-3 pt-2 text-surface-900"
                    >
                        <Button
                            @click="() => onSubmit()"
                            severity="success"
